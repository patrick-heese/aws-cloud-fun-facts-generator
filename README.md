...
3. Navigate to the `cloudformation` folder, build, and deploy:  
    ...
    cd cloudformation
    sam build
    sam deploy --guided --capabilities CAPABILITY_NAMED_IAM --tags Project=aws-cloud-fun-facts-generator
    ```
...
> **Note**: Ensure the AWS CLI user (`aws configure`) or CloudFormation assumed role has sufficient permissions to manage **Lambda**, **DynamoDB**, **API Gateway**, **AWS Bedrock**, and **IAM resources**.  
